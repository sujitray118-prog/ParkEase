...
const SPOT_IDS = ROWS.flatMap(row =>
  Array.from({ length: SPOTS_PER_ROW }, (_, i) => `${row}${i + 1}`)
);
...
function isConfigured() {
  return Boolean(FIREBASE_CONFIG.apiKey) && FIREBASE_CONFIG.apiKey !== 'YOUR_API_KEY';
}
...
function emptySpot() {
  return { status: 'available', bookedBy: null, vehicle: null, sessionId: null, reservedAt: null, enteredAt: null };
}
